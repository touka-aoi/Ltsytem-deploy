<script lang="ts">
	...
	let { id, Ltname, desc, maxMem, holdDay, holdHour, assignNum, holdPlace, speakers, status } =
		data.Lt;
	let { username, isSpeaker, isViewer } = data.user;
					...
					{#if !isViewer}
						<form method="post" action="?/register">
							<input
								...
								value="登録する"
							/>
							<input type="hidden" name="Ltname" bind:value={Ltname} disabled={isViewer}/>
						</form>
					{:else}
				...
				{#each speakers as { username, avatarData, LtTitle, LtComment, LtLink }}
					<div class="flex gap-14 bg-zinc-50 roudned-sm py-5 px-10">
						<div class="flex flex-col justify-center items-center gap-3">
							{#if avatarData}
								{#await account.downloadAvatar(avatarData) then avatar}
									<div
