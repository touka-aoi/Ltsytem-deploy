<script lang="ts">
	...
	let username = user;
	let avatarFilename: string | null = avatarUrl; // ファイル名
	...
	// Image用の変数
	let files: FileList | null = null; // inputからの入力
	let file: File | null = null; // ファイル
	...
	async function updateProfile() {
		try {
			...
			// ファイル更新
			if (file != null && avatarFilename != null) {
				const error = await accountRequest.AvatarUpload(file, avatarFilename);
				if (error) throw Error(error);
			}

			// ファイル削除
			if (avatarUrl != null && avatarData) {
				const error = await accountRequest.deleteAvatar(avatarUrl);
				if (error) throw Error(error);
			}

			const { error } = await accountRequest.upsertProfile(userId, username, avatarFilename);
	...
	onMount(async () => {
		if (avatarUrl) {
			const { fileUrl, error } = await accountRequest.downloadAvatar(avatarUrl);
			avatarData = fileUrl;
		}
	});
</script>
