...
<div class="bg-slate-100 rounded px-5 py-4">
	<div class="mb-2 gap-4">
		<a href="{base}/lts/{LtData.data.LtID}" class="flex flex-col gap-1 bg-slate-50 p-4 rounded shadow-md">
			...
			<div>
				<p class="text-2xl">{LtData.data.Ltname}</p>
			</div>
		</a>
	</div>
	...
	{#if canRevise}
		<div class="flex gap-2 justify-between	mt-4">
			<a href="/lts/{LtData.data.LtID}/register" class="w-fit">
				<button class="minibtn bg-yellow-200 shadow-md">修正する</button>
			</a>
			<a href={LtData.data.holdPlace} class="w-fit">
